...
for i=1:Options.TranslationIterations+Options.AffineIterations
    ...
    I_error = I_template - I_warped;  % DELETE
    ...
    G_x_warped = affine_transform_2d_double(G_x,x,y,W_xp);  % DELETE. Similar to step 1:
    G_y_warped = affine_transform_2d_double(G_y,x,y,W_xp);  % DELETE
    ...
    if(i>Options.TranslationIterations) 
        ...
        H = zeros(6,6);
        for j=1:NumPixelsTemplate
            H = H + I_steepest(j,:)'*I_steepest(j,:);  % DELETE the product
        end

        % 7: Computer sum_x [gradI*dW/dp]^T (T(X)-I(W(x;p))]
        sum_xy = zeros(6,1);
        for j=1:NumPixelsTemplate
            sum_xy = sum_xy + I_steepest(j,:)'*I_error(j);  % DELETE the product
        end

        % 8: Computer delta_p by solving the linear system in Equation 10
        delta_p = H\sum_xy;  % DELETE
