...
% Data from exercise 6 for testing (first frame)

K = load('testdata/K.txt');
keypoints = load('testdata/keypoints.txt')';
p_W_landmarks = load('testdata/p_W_landmarks.txt')';

% prevState = [keypoints; p_W_landmarks];



prevImage = imread(sprintf('testdata/%06d.png',0));
...
for i = 1:20
    ...
    currImage = imread(sprintf('testdata/%06d.png',i));
    
    newkp= sum(...
        ~ismember(prevState(3:5,:)',p_W_landmarks(1:3,:)', 'rows'));
    
    [ currState, currPose, tempState ] = processFrame_new_alex(...
    ...
    if (numel(R_C_W) > 0)
        ...
        %scatter3(currState(3, end-newkp:end), ...
        %         currState(4, end-newkp:end), ...
                 currState(5, end-newkp:end), 5);
        view(0,0);
        hold off
    else
